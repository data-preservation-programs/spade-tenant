...
func (a *apiV1) handleSetReplicationConstraints(c echo.Context) error {
	...
	err = a.db.Transaction(func(tx *gorm.DB) error {
		for i, replicationConstraint := range replicationConstraints {
			replicationConstraints[i].CollectionID = uuid.MustParse(c.Param("collectionUUID"))
			constraint := &db.ReplicationConstraint{TenantID: db.ID(GetTenantContext(c).TenantID), CollectionID: uuid.MustParse(c.Param("collectionUUID")), ConstraintID: replicationConstraint.ConstraintID, ConstraintMax: replicationConstraint.ConstraintMax}
...
func (a *apiV1) handleDeleteReplicationConstraints(c echo.Context) error {
	...
	err = a.db.Transaction(func(tx *gorm.DB) error {
		for _, constraintId := range ids {
			if constraintId > 0 {
				res := a.db.Delete(&ReplicationConstraint{TenantID: db.ID(GetTenantContext(c).TenantID), CollectionID: uuid.MustParse(c.Param("collectionUUID")), ConstraintID: db.ID(constraintId)})
...
func (a *apiV1) handleGetReplicationConstraints(c echo.Context) error {
	...
	res := a.db.Model(&db.ReplicationConstraint{TenantID: db.ID(GetTenantContext(c).TenantID)}).Find(&replicationConstraints)
